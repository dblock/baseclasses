...
/*
   ...
   context.
*/
void CMd5::Md5Update(Md5_CTX * context, const unsigned char * const input, unsigned int inputLen) {
...
/*
   Encodes input (unsigned long int4) into output (unsigned char). Assumes len is
   a multiple of 4.
*/
...
void CMd5::Md5Calculate(const CString& String, CString * pResult){
	Md5_CTX context;
    pResult->SetSize(17);	
	Md5Init (& context);
	Md5Update (& context, (const unsigned char *) String.GetBuffer(), String.GetLength());
	Md5Final ((unsigned char *) pResult->GetBuffer(), & context);
	pResult->SetLength(16, 0, false);
}

void CMd5::Rc4PrepareKey(unsigned char *key_data_ptr, int key_data_len, rc4_key *key) {
...
void CMd5::Rc4Calculate(unsigned char *buffer_ptr, int buffer_len, unsigned char * pass_key) {
	unsigned char x;
	unsigned char y;
	unsigned char* state;
	...
	rc4_key key;
	Rc4PrepareKey(pass_key, base_strlen((char *)pass_key), &key);
...
CString CMd5::Md5(const CString& Target){
    CString Result;
    Md5Calculate(Target, & Result);
    return Result;
}
